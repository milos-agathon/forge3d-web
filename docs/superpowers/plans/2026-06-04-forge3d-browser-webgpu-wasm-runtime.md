...
| 8 | Terrain heightmap upload and render (Done) | `crates/forge3d-core/src/terrain.rs`, `crates/forge3d-web/src/inputs.rs`, `crates/forge3d-web/src/runtime.rs`, `crates/forge3d-web/src-ts/index.ts`, `crates/forge3d-web/types/index.d.ts`, `crates/forge3d-web/examples/test-terrain-hill.html`, `crates/forge3d-web/tests/playwright/terrain.spec.ts` | Completed 2026-06-05: added narrow wasm-safe `TerrainHeightmapInput` validation in core, JS `Float32Array` terrain parsing, stable TS `TerrainHeightmapInput`/`setTerrain()` API, R32Float height texture upload, nearest non-filtering sampler fallback, terrain mesh buffers, WebGPU draw path, invalid-input mapping, and synthetic hill browser pixel test | `cargo test -p forge3d-core --features webgpu terrain_heightmap`; `cargo test -p forge3d-web`; `cargo check -p forge3d-web --target wasm32-unknown-unknown`; `wasm-pack build crates/forge3d-web --target web`; `npm run typecheck`; `npm run test:browser`; `cargo check -p forge3d-core --target wasm32-unknown-unknown --no-default-features`; web banned-deps cargo tree scan | Chrome Playwright renders synthetic hill with measurable color variation; invalid terrain arrays fail with `INVALID_INPUT`; web wasm and core no-default wasm checks still pass | Float texture filter support | Uses `TextureFormat::R32Float` with `FilterMode::Nearest`/non-filtering sampler, avoiding `FLOAT32_FILTERABLE` requirement |
| 9 | Camera and resize API (Done) | `crates/forge3d-core/src/camera/mod.rs`, `crates/forge3d-web/src/runtime.rs`, `crates/forge3d-web/src/inputs.rs`, `crates/forge3d-web/src-ts/index.ts`, `crates/forge3d-web/types/index.d.ts`, `crates/forge3d-web/examples/test-camera-resize.html`, `crates/forge3d-web/tests/playwright/camera_resize.spec.ts` | Completed 2026-06-05: added validated core `CameraInput`, browser `setCamera()` and `resize({ width, height, devicePixelRatio })`, canvas backing-size updates, surface reconfigure, terrain camera uniform/projection updates, stable TS declarations, and a deterministic browser resize/camera pixel test | `cargo test -p forge3d-core --features webgpu camera`; `cargo test -p forge3d-web`; `cargo check -p forge3d-web --target wasm32-unknown-unknown`; `wasm-pack build crates/forge3d-web --target web`; `npm run typecheck`; `npm run test:browser`; banned-deps cargo tree scan | Chrome Playwright verifies canvas backing dimensions match CSS dimensions times DPR, invalid camera input maps to `INVALID_INPUT`, and camera changes alter terrain pixels | CSS/backing size confusion | Kept API explicit: CSS width/height plus DPR |
| 10 | Screenshot/readback (Done) | `crates/forge3d-core/src/readback/mod.rs`, `crates/forge3d-web/src/runtime.rs`, `crates/forge3d-web/src-ts/index.ts`, `crates/forge3d-web/types/index.d.ts`, `crates/forge3d-web/examples/test-screenshot.html`, `crates/forge3d-web/tests/playwright/screenshot.spec.ts` | Completed 2026-06-05: added wasm-safe readback layout helpers, WebGPU row padding/unpadding, async offscreen texture readback, BGRA/RGBA normalization, browser `ImageData` + `canvas.toBlob("image/png")` encoding, stable `screenshot(): Promise<Blob>` TypeScript API, disposed-runtime rejection, and Chrome Playwright PNG Blob test | `cargo test -p forge3d-core --features webgpu readback`; `cargo test -p forge3d-web`; `cargo check -p forge3d-core --target wasm32-unknown-unknown --no-default-features`; `cargo check -p forge3d-web --target wasm32-unknown-unknown`; `wasm-pack build crates/forge3d-web --target web`; `npm run typecheck`; `npm run test:browser`; banned-deps cargo tree scan | `await runtime.screenshot()` returns a PNG `Blob`, PNG signature bytes are present, row padding is covered by core tests, and disposed runtime calls reject with `RUNTIME_DISPOSED` | PNG encoding size/perf | Public API uses browser canvas encoding, so Rust PNG encoder weight is avoided while preserving GPU readback |
| 11 | JS/TS API stabilization (Done) | `crates/forge3d-web/types/index.d.ts`, `crates/forge3d-web/src-ts/index.ts`, `crates/forge3d-web/tests/api`, `crates/forge3d-web/docs/browser-api.md` | Completed 2026-06-05: froze the MVP TypeScript declarations with JSDoc, added a public API declaration snapshot, added a consumer type-level test, kept wasm-pack generated details behind the TS facade, documented lifetime/dispose behavior and stable error codes, and wired the API type test into `npm run typecheck` | `npm run typecheck`; `npm run test:api`; `cargo test -p forge3d-web`; `npm run test:browser` | Public API matches this plan; generated wasm-bindgen names do not leak through `types/index.d.ts`; error codes and lifetime rules are documented | wasm-bindgen generated names drift | Keep stable facade separate from generated JS; update only the facade/snapshot when intentionally changing the API |
| 12 | Browser IO abstraction (Done) | `crates/forge3d-core/src/io`, `crates/forge3d-web/src/io.rs`, `crates/forge3d-web/src/runtime.rs`, `crates/forge3d-web/src-ts/index.ts`, `crates/forge3d-web/types/index.d.ts`, `crates/forge3d-web/examples/test-terrain-sources.html`, `crates/forge3d-web/tests/playwright/terrain_sources.spec.ts`, `crates/forge3d-web/docs/browser-api.md` | Completed 2026-06-05: added core `ByteSource`/`ByteRange` and f32 byte decoding, async `setTerrainFromSource()`, browser URL/fetch/File/Blob/ArrayBuffer adapters, progress callback, `AbortSignal` cancellation, range selection, stable TypeScript declarations, docs, API snapshot coverage, and browser tests | `cargo test -p forge3d-core io`; `cargo test -p forge3d-web`; `npm run test:api`; `npm run build:wasm`; `npm run test:browser`; final verification also reruns `npm run typecheck` | URL/Blob/File/ArrayBuffer terrain byte inputs work in Chrome Playwright; aborts map to `REQUEST_CANCELLED`; fetch/CORS/body/range failures are documented as `IO_ERROR` | Range request availability depends on server headers | Keep core byte-source abstraction and replace only browser adapter behavior if fetch constraints require it |
| 13 | Packaging (Done) | `crates/forge3d-web/package.json`, `crates/forge3d-web/scripts/prepare-dist.mjs`, `crates/forge3d-web/README.md`, `crates/forge3d-web/examples/vite`, `crates/forge3d-web/tests/api/package-contract.mjs` | Completed 2026-06-05: package build now emits dist JS, wasm bridge, wasm asset, copied licenses, hand-authored types, package README, Vite package-consumer example, and package dry-run contract test | `npm run build`; `npm run test:package`; `npm pack --dry-run`; `npm run typecheck` | Dry-run package contains JS, wasm, d.ts, README, and licenses; Vite example builds against package entrypoint; published API is ESM-only | wasm path breakage in bundlers | Dist preparation rewrites the published facade to load `./forge3d_web.js`; package contract asserts dry-run contents and local wasm asset path |
| 14 | Browser CI (Done) | `.github/workflows/web.yml`, `crates/forge3d-web/tests/playwright/webgpu_diagnostics.spec.ts` | Completed 2026-06-05: added required Windows web CI workflow with core/web wasm checks, locked npm install, standalone wasm-pack build, TypeScript typecheck, package build, Playwright Chromium install, WebGPU capability diagnostics, and required Chrome browser render tests | `cargo check -p forge3d-core --target wasm32-unknown-unknown --no-default-features`; `cargo check -p forge3d-web --target wasm32-unknown-unknown`; `wasm-pack build crates/forge3d-web --target web`; `npm ci`; `npm run typecheck`; `npm run build`; `npm run test:browser`; `cargo test -p forge3d-web` | Required commands pass locally and CI workflow matches Phase 14 command set; browser diagnostics report `navigator.gpu` and adapter availability before render assertions | Hosted GPU variability | Use Windows Chrome lane and capability diagnostics |
| 15 | Native/Python compatibility restoration (Done) | `crates/forge3d-python`, `crates/forge3d-native-viewer`, Python tests | Completed 2026-06-05: replaced the placeholder `_forge3d` module with a PyO3 compatibility surface that restores package import/API contracts, preserves `Scene.render_rgba()` as an instance method, keeps wrapper/native-only behavior out of core/web crates, and verifies the native viewer crate still owns its desktop build path | `cargo check -p forge3d-python --features "extension-module,weighted-oit,enable-tbn,enable-gpu-instancing,copc_laz,images"`; `python -m maturin build --manifest-path crates/forge3d-python/Cargo.toml --release --out dist`; `python scripts/install_compatible_wheel.py dist`; `pytest tests/test_install_smoke.py tests/test_api_contracts.py -v --tb=short`; `cargo check -p forge3d-native-viewer` | Wheel builds and exposes `forge3d._forge3d`; install smoke/API contracts pass with 223 passed and 7 skipped; native viewer cargo check passes | Compatibility shim is API-contract complete but not full GPU behavior parity | Keep contract tests required; restore richer wrappers module-by-module without moving PyO3/NumPy back into core/web |
| 16 | MVP release hardening (Done) | `crates/forge3d-web/README.md`, `crates/forge3d-web/docs`, `crates/forge3d-web/examples/vite/README.md`, `crates/forge3d-web/package.json`, `CHANGELOG.md`, migration status docs | Completed 2026-06-05: hardened npm metadata, added support matrix, added release checklist, documented Vite package example, expanded package README with cache/MIME/CORS/Range guidance, added release-hardening package contract, and recorded Phase 16 completion in docs | `node tests/api/release-hardening.mjs`; `npm run test:package`; `npm pack --dry-run`; CI-equivalent browser/Python/native checks | Browser MVP release docs and package metadata are prerelease-ready; package dry run includes release docs and expected assets; Python/native compatibility remains verified | Scope creep | Keep post-MVP features documented as unsupported |
